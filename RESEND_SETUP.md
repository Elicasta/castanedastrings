...
- `INQUIRY_TO_EMAIL` is where form submissions are delivered. Use one email, or multiple emails separated by commas.
- Optional: `INQUIRY_CC_EMAILS` and `INQUIRY_BCC_EMAILS` also support comma-separated email lists.
...
After editing recipients, redeploy the Vercel project so the serverless function receives the updated environment variables.
